<template>
  <view class="new-users copy-data container_mini">
    <HeadPlaceHolder></HeadPlaceHolder>
    <view class="bg_header bg-color"></view>
    <scroll-view-page class="relative">
      <view class="head">
          ...
          <view class="user-info">
            <view style="align-self:center">
              ...
              <button class="bntImg" v-if="!avatarUrl"  @click="goEdit()">
                <image class="avatar" src='../../../static/image/未标题-1.png'></image>
              ...
              <view v-if="avatarUrl" style="
							...
							border-radius: 50%;
							background: #fff;">
                <image class="avatar" :src='avatarUrl' @click="goEdit()"></image>
                <!-- <image v-else class="avatar" :src="mineUserInfo.avatarUrl" mode="" @click="goEdit()"></image> -->
              </view>
            </view>

            <view class="info">
              <!-- #ifdef MP -->
              <view class="name" v-if="!mineUserInfo.openId" @tap="openAuto"
                style="display: flex; align-items: center;height: 100%; font-size: 30rpx;">
                请点击授权
              </view>
              <!-- #endif -->
              <view class="name" v-if="mineUserInfo.openId" @tap="goEdit()">
                <view class="acea-row row-middle marginBottom16">
                  <view style="margin-right: 8rpx">{{mineUserInfo.realname || '微信用户'}}</view>
                  <image src="@/static/image/edit.png" mode="aspectFit" @click="goEdit()"></image>
                </view>
                <view v-if="mineUserInfo.company_name" class="company-txt">{{mineUserInfo.company_name}}</view>
              </view>
            </view>
          </view>
        </view>
...
<script>
  ...
  // import BookOrder from "./component/BookOrder.vue"
  import {
    HTTP_REQUEST_URL
  } from "@/config/app.js";
  import {
  ...
  export default {
    ...
    watch: {},
    computed: mapGetters(['isLogin', 'dictionary']),
    onLoad(options) {
      // #ifdef MP-WEIXIN
      uni.hideHomeButton()
      // #endif
    ...
    methods: {
      ...
      getUserInfo: async function() {
        ...
        await getUserInfo().then(res => {
          ...
          this.avatarUrl = IMG_HTTP_REQUEST_HEAD + IMG_REQUEST_URL +res.data.avatar
        });
      },
      ...
      async getUserProfile() {
        ...
        let self = this;
        await Routine.getUserProfile()
          .then(res => {
            Routine.getCode()
              .then(code => {
                let mineUserInfo = res.userInfo;
                ...
                this.avatarUrl = mineUserInfo.userInfo.avatar
                this.$Cache.set("AVATAR", mineUserInfo.userInfo.avatar)
              })
              .catch(res => {
                uni.hideLoading();
              });
      ...
      goEdit() {
        uni.navigateTo({
          url: `/pages/users/userInfo/index?${this.$util.convertObjectToString(this.mineUserInfo)}`
        })
      },
      goBoatSchduled(item) {
        ...
        uni.navigateTo({
          url: `/pages/users/schduledList/index?${this.$util.convertObjectToString(obj)}`
        })
      },
      goOutside() {
        uni.navigateTo({
          url: `/pages/users/aboutBoat/index?src=${this.outsideUrl}`
        });
      }
    },
    onReachBottom() {}
...
<style lang="scss">
  .new-users {
    ...
    .backgroundLine {
      background-image: linear-gradient(172deg, #ffeded, #fef1f5, #fdf6fa, #fdfbfe, #ffffff);
    }

    .head {
      position: relative;
      padding: 124rpx 28rpx 46rpx 28rpx;
