	...
	<view class="new-users lightGreyBg height100vh flex flex-col">
		...
		<view class="relative zIndex1 overflowAuto flex1 marginBottom24">
			<view class="formBuilder height100">
				...
				<view class="form-line" style="margin-bottom: 25rpx !important;">
					<label for="">是否为管理员</label>
					<picker class="marginTop24" style="width: 100%;" mode="selector" :range="isAdminList" :value="adminIndex"
						@change="changeAdmin">
						<view class="acea-row row-middle justify-between">
							<view class="uni-input">{{isAdminList[adminIndex]}}</view>
							<image src="../../../static/asset/矩形(4).png" class="flag" mode="aspectFit">
							</image>
						</view>
					</picker>
	...
	export default {
		...
		methods: {
			...
			selectSearch(v) {
				...
				this.dataLists = this.activeIndex ? this.job.items : this.post.items
				...
				tempFiles.forEach(item => {
					this.$util.uploadFile(item).then(res => {
						...
						if (jsonData.code == 200) {
							self.employeeData.avatar = item.name
						} else {
							uni.showToast({
								title: "文件上传出现异常错误",
								icon: "none"
